...
public final class FreecamTranslator implements ResourceManagerReloadListener
{
	 ...
	 * or into English if the "Force English" setting is enabled. Both WI
	 * Freecam and vanilla translations are supported.
	 */
	 ...
	 * the current language. Both WI Freecam and vanilla translations are
	 * supported.
	 */
	 ...
	 * of the current language. Does not include any of WI Freecam's
	 * translations.
	 */
	...
	public Map<String, String> getWiFreecamsCurrentLanguage()
	{
		...
		Iterable<String> langCodes, BiConsumer<String, String> entryConsumer)
	{
		for(String langCode : langCodes)
		{
			...
			for(Resource resource : manager.getResourceStack(langId))
				...
				{
					if(isBuiltInWiFreecamResourcePack(resource))
						Language.loadFromJson(stream, entryConsumer);
					
				}catch(IOException | JsonParseException e)
				{
					System.out
						.println("Failed to load WI Freecam translations for "
							+ langCode);
					e.printStackTrace();
				...
				{
					System.out.println(
						"Unexpected exception while loading WI Freecam translations for "
							+ langCode);
					e.printStackTrace();
				}
		}
	...
	/**
	 * Ensures that the given resource is from WI Freecam's built-in resource
	 * pack, or at least from another client-side mod pretending to be WI
	 * Freecam, as should be impossible for server-provided resource packs to
	 * obtain a KnownPack of <code>fabric:wi_freecam</code>.
	 */
	private boolean isBuiltInWiFreecamResourcePack(Resource resource)
	{
		...
		return ("fabric".equals(knownPack.namespace())
			...
			&& "wi_freecam".equals(knownPack.id());
	}
}
